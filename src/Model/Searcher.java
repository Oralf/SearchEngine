...
public class Searcher {
    ...
    public Searcher(boolean withSemantic, boolean withEntitiesSearch,boolean withStemmer,String postFilePath,Indexer indexer) {
    ...
    public LinkedHashMap<String,List<Pair<String,Double>>> searchSingleQuery(String query, String queryID, String desc){
        ...
        if(withSemantic){
            try {
                ...
                for (int i = 0; i < wordsInQuery.length; i++) {
                    ...
                    int score;
                    Document wordDDoc;
                    for(int j=0; j+1<wordsScore.length;j=j+2){
                        score=Integer.parseInt(wordsScore[j+1]);
                        if(score>150) {// good word
                            wordDDoc = indexer.getParser().parseText(wordsScore[j], "defualt");
                            for (String w : wordDDoc.getDocMap().keySet()) {
                                if (score < 1000) {
                                    queryScore.put(w, 0.1);
                                } else if (score < 5000) {
                                    queryScore.put(w, 0.4);
                                } else {// score over 5000
                                    queryScore.put(w, 0.6);
                                }
                            }
                        }
                        else{// no more scores over 100
                            break;
                        }
                    }
        ...
        double sumQueryRank=queryScore.size();
    ...
    public LinkedHashMap<String,LinkedHashMap<String, List<Pair<String, Double>>>> searchFileQuery(String queryFilePath) {
        ...
        try {
            ...
            while(it.hasNext()) {
                ...
                System.out.println("Query running:"+queryText);
